    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.sessions.store") }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <textarea class="form-control ckeditor {{ $errors->has('topics') ? 'is-invalid' : '' }}" name="topics" id="topics">{!! old('topics') !!}</textarea>
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('zoom_link') ? 'is-invalid' : '' }}" name="zoom_link" id="zoom_link" required>{{ old('zoom_link') }}</textarea>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('next_session') ? 'is-invalid' : '' }}" name="next_session_id" id="next_session_id">
                    @foreach($next_sessions as $id => $entry)
                        <option value="{{ $id }}" {{ old('next_session_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
