    ...
    <div class="row justify-content-center">
        <div class="col-md-12">
            ...
            <div class="card">
                ...
                <div class="card-body">
                    <form method="POST" action="{{ route("frontend.schedules.update", [$schedule->id]) }}" enctype="multipart/form-data">
                        @method('PUT')
                        @csrf
                        ...
                        <div class="form-group">
                            ...
                            <input class="form-control date" type="text" name="date" id="date" value="{{ old('date', $schedule->date) }}" required>
                            @if($errors->has('date'))
                                <div class="invalid-feedback">
                                    {{ $errors->first('date') }}
                                </div>
                        ...
                        <div class="form-group">
                            ...
                            <input class="form-control timepicker" type="text" name="time" id="time" value="{{ old('time', $schedule->time) }}" required>
                            @if($errors->has('time'))
                                <div class="invalid-feedback">
                                    {{ $errors->first('time') }}
                                </div>
                        ...
                        <div class="form-group">
                            ...
                            @foreach(App\Models\Schedule::STATUS_RADIO as $key => $label)
                                <div>
                                    <input type="radio" id="status_{{ $key }}" name="status" value="{{ $key }}" {{ old('status', $schedule->status) === (string) $key ? 'checked' : '' }}>
